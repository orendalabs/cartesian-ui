...
export class AuthorizationSandbox extends Sandbox {
    ...
    roleFetchData$ = this.store.pipe(
        select(selectors.getRoleFetchData)
    )
    rolesFetchData$ = this.store.pipe(
        select(selectors.getRolesFetchData)
    )

    rolesFetchMeta$ = this.store.pipe(
        select(selectors.getRolesFetchMeta)
    )
    ...
    assignRole = (roleForm: ManageRoleForm): void => {
        this.store.dispatch(roleActions.doAssignRole({ roleForm: roleForm }))
    }

    revokeRole = (roleForm: ManageRoleForm): void => {
        this.store.dispatch(roleActions.doRevokeRole({ roleForm: roleForm }))
    }

    fetchRoles = (requestCriteria: RequestCriteria<SearchRoleForm>): void => {
        this.store.dispatch(roleActions.doFetchRoles({ requestCriteria: requestCriteria }))
    }

    fetchRoleById = (id: string) => {
        this.store.dispatch(roleActions.doFetchRole({id: id}))
    }

    createRole = (form: CreateRoleForm) => {
        this.store.dispatch(roleActions.doCreateRole({form: form}));
    }

    deleteRoleById = (id: string) => {
        this.store.dispatch(roleActions.doDeleteRole({id: id}))
    }
    ...
    fetchPermissions = (requestCriteria: RequestCriteria<SearchPermissionForm>): void => {
        this.store.dispatch(permissionActions.doFetchPermissions({requestCriteria: requestCriteria}))
    }

    fetchPermissionById = (id: string) => {
        this.store.dispatch(permissionActions.doFetchPermission({id: id}))
    }
}