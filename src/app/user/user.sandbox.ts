...
export class UserSandbox extends Sandbox {
  ...
  public deleteUserById(id: string): void {
    this.store.dispatch(
      userActions.doDeleteUser({ id: id })
    );
  }
  ...
  public createUser(form: AdminUserCreateForm): void {
    this.store.dispatch(
      userActions.doCreateUser({ form: form })
    );
  }
  ...
  public updateUser(id: string, form: EditUserForm): void {
    this.store.dispatch(
      userActions.doUpdateUser({ id: id, form: form })
    );
  }
  ...
  public fetchProfile(token: string): void {
    this.store.dispatch(
      userActions.doFetchAuthenticatedUser({ token: token })
    );
  }
